docsTemplateEngine.directive('docsTemplateEngine', () => {
  return {
    link: function (scope, element, attrs) {
      scope.data = scope[attrs["docsTemplateEngine"]];
    },
    restrict: "A",
    template: "<div class='row justify-content-between'>" +
      ...
      "<div>{{t.name}}</div>" +
      "<div class='p-1' ng-if='cur.templateName === t.name'>" +
      "<hr>" +
      "<textarea class='form-control col' rows='5' ng-model='cur.data'></textarea>" +
      "<button class='mx-1 my-1 btn btn-success' ng-click='updateData(cur.data)'>Изменить</button>" +
      "<textarea class='form-control col' rows='10' ng-model='cur.template'></textarea>" +
      "<button class='mx-1 my-1 btn btn-success' ng-click='updateTemplate(cur.template)'>Изменить</button>" +
      ...
      "</div>" +
      "<button class='col btn btn-success my-1' ng-class='{disabled : !cur.data}' ng-click='createDocs(t)'>Создать" +
      "экземпляры</button>" +
